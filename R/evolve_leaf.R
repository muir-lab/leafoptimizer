...
#' 
#' @param leaf_traits A vector of leaf traits to optimize. Must one or more of stomatal conductance (\code{g_sw}), stomatal ratio (\code{sr}), and leaf size (\code{leafsize}). Partial matching allowed.
...
evolve_leaf <- function(leaf_traits, constraints, leaf_par, enviro_par, constants, ...) {

  ##### Initial guess at traits -----

  ##### Find traits that maximize fitness -----
  soln <- optim(leaf_traits, leaf_fitness, constraints, leaf_par, enviro_par, constants, return_negative = TRUE, ...)
